...
function Study() {
  ...
  // Auto-play pronunciation whenever a new card is shown — only after session starts
  useEffect(() => {
    if (started && cards.length > 0 && !finished) {
      const card = cards[currentIndex]
      speak(card.gender ? `${card.gender} ${card.german_word}` : card.german_word)
    }
  }, [currentIndex, cards, started])
  ...
  const reviewCount = cards.filter(c => c.last_reviewed !== null).length
  const newCount = cards.filter(c => c.last_reviewed === null).length
  ...
  if (!started) return (
    <div className="text-center mt-20 max-w-sm mx-auto">
      ...
      <div className="grid grid-cols-2 gap-3 mb-8">
        <div className="bg-blue-900/30 border border-blue-800/40 rounded-xl p-4 text-center">
        ...
        <div className="bg-purple-900/30 border border-purple-800/40 rounded-xl p-4 text-center">
          <div className="text-3xl font-bold text-purple-400">{newCount}</div>
          <div className="text-xs text-gray-400 mt-1">New words</div>
        </div>
      </div>

      <button
        onClick={() => setStarted(true)}
        className="bg-blue-600 hover:bg-blue-500 text-white px-10 py-3 rounded-xl font-semibold text-lg transition-all active:scale-95 shadow-lg shadow-blue-900/40"
      >
        Start Session
      </button>
    </div>
